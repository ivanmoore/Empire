...
public class TerrainInterpolator_after {
    ...
    private void doSquares(int cellSize) {
        points(cellSize).forEach(point -> doSquare(point, cellSize + 1));
    }

    private void doDiamonds(int cellSize) {
        points(cellSize).forEach(point -> doDiamond(point, cellSize + 1));
    }

    private List<Point> points(int cellSize) {
